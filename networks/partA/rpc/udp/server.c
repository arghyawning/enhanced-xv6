...
//     char last_input;
//     char name;
//     struct sockaddr_in client_addr; // Add client address information
// } ClientInfo;
...
//         clients[i].client_sock = -1; // Since UDP is connectionless, we don't have a dedicated socket for each client
...
int main()
{
    ...
    while (1)
    {
        ...
        if (flag)
        {
            ...
            sendto(server_sock, result, strlen(result), 0, (struct sockaddr *)&clients[0].client_addr, sizeof(struct sockaddr_in));
            break;
        }
